...
NAE *NatAmbio::newNAE(struct s_nae* n_nae)
{
  if(!quiet) {
    ...
    if(n_nae->projection)
      std::cout << "NatAmbio: NAE projection on, gain " << n_nae->proy_gain
                << ", " << ((n_nae->proy_mode == 0) ? "delayed" : "undelayed")
                << " into the ambience" << std::endl;
  }
  NAE *n_nae_p;
  ...
  n_nae_p->setProjection(n_nae->projection, n_nae->proy_gain, n_nae->proy_mode);
