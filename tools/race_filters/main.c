...
/* No static: process() en race.c también la llama (forward-declarada allí). */
...
/* save_race_wavs — escribe los dos filtros RACE finales al disco.
 * Nombrado idéntico al de ambio_filters_scipy.py (contrato con new_ambio_filter.sh):
 *   RACE_{AZ:02}_deg_ITD_{ITD_us}_micsec_ILD_{ILD:.1f}_dB_a_{ALPHA:.1f}_{direct|cross}.wav
 */
static int save_race_wavs(const double *direct, const double *cross,
                          int itd_us, double ild_db, double ild_alpha,
                          int azimuth_deg, int sample_rate, int filter_len) {
    char path[512];
    snprintf(path, sizeof(path),
             "filters/RACE_%02d_deg_ITD_%d_micsec_ILD_%.1f_dB_a_%.1f_direct.wav",
             azimuth_deg, itd_us, ild_db, ild_alpha);
    int rc1 = write_wav(path, direct, filter_len, sample_rate);
    snprintf(path, sizeof(path),
             "filters/RACE_%02d_deg_ITD_%d_micsec_ILD_%.1f_dB_a_%.1f_cross.wav",
             azimuth_deg, itd_us, ild_db, ild_alpha);
...
int main(int argc, char **argv) {
    /* Valores por defecto idénticos al script Python */
    int    itd_us      = 170;
    double ild_db      = 14.0;
    ...
    /* Mismo parser flag-luego-valor que el Python (no GNU getopt para
     * preservar exactamente la semántica del script original). */
    const char *nextarg = "";
    ...
    /* Los buffers de salida RACE viven en main; process los rellena,
     * save_race_wavs los persiste, y main los libera. */
    double *direct = calloc((size_t)filter_len, sizeof(double));
    double *cross  = calloc((size_t)filter_len, sizeof(double));
    if (!direct || !cross) {
        fprintf(stderr, "Memory allocation failed for RACE buffers\n");
    ...
    int rc = process(itd_us, ild_db, ild_alpha, azimuth, sample_rate, filter_len,
                     direct, cross);
    if (rc == 0) {
        rc = save_race_wavs(direct, cross, itd_us, ild_db, ild_alpha,
