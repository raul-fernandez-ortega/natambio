...
Standalone command-line generator for **XTC** (crosstalk cancellation) FIR
filters. It writes the *direct* and *cross* filter pair as 32-bit float WAV
files under `./filters/`.

It reuses the project's shared filter-design code in `../../lib`
(`xtc.c` → `dsp.c`, `binaural_cues.c`) — the very same units `natambio` links —
