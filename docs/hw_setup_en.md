# How to Build a NatAmbio System
...
NatAmbio has a brain —the DSP PC that hosts the software and all system control— and lungs —the audio interface that carries the stereo signal to the brain for processing and returns the result to the amplifier and loudspeakers. We will describe both elements and explain how they connect to the rest of the sound system.

## A PC for DSP/NatAmbio

First you need a PC running the GNU/Linux operating system (recommendation: Debian distribution). Since it will be immersed in a sound system, one of its requirements is that it be silent —that it run without noise. There are many systems with low-noise fans, but surely the best option is a passively cooled PC. In that case, noise is literally zero. Traditionally, passively cooled equipment was associated with low-power processors. However, today's situation is very different and there are solutions capable of running NatAmbio with ample resource headroom.
...
Of course, at least my home NatAmbio system has no screen, mouse or keyboard. It runs autonomously with NatAmbio and jackd running as services under systemd. System administration and software updates are performed remotely through SSH. For that, my PC is connected by ethernet to the home network.

## Audio interfaces for NatAmbio

NatAmbio's audio interface is the gateway through which audio enters and leaves the DSP PC. A quality interface ensures a quality audio system. By quality we mean very low distortion, flat frequency response, very low background noise, and sufficient input and output levels.

Personally, I use FireWire audio interfaces. Although FireWire is now an obsolete technology, that very obsolescence has created an extraordinarily attractive second-hand market for NatAmbio. Many very high quality professional interfaces can be found at reduced prices and continue to work perfectly under GNU/Linux through FFADO. It is possible to equip a NatAmbio system with an excellent audio interface at a very reasonable cost.

I have acquired, and tested, several audio interfaces compatible with the FireWire audio library for GNU/Linux, [FFADO](https://ffado.org/), and, correctly configured, they operate for days without a single xrun. Right now I have two Echo Audiofire4, which I have even connected in a daisy chain without problems, an Edirol FA-66 and an Edirol FA-101. All four work perfectly. The Echo interfaces have software controls manageable through ffado-dbus-service. The Edirol interfaces have their controls on the interface itself.

To have a FireWire interface in the PC, I have installed a PCIe-to-FireWire interface on the motherboard, which is currently easy to find at low cost.

The most practical current option is to have a USB audio interface. I have tested the correct operation of NatAmbio with a very simple interface of reasonable quality from Behringer, the UMC204HD, and with a high quality card, the Focusrite Scarlett 6i6. In the case of the Scarlett interface it is essential to install [alsa-scarlett-gui](https://github.com/geoffreybennett/alsa-scarlett-gui) to be able to activate it and configure all its routing and level controls. The Focusrite Scarlett interface is very stable for days with jackd, as is the UMC204HD interface.
...
| Focusrite Scarlett 6i6 | USB | 4 Out + SPDIF | Very stable, good quality, ideal for NatAmbio | Requires alsa-scarlett-gui | Recommended modern system |
| Edirol FA-101 | FireWire |  8 Out + SPDIF | Excellent quality, cheap second-hand | Requires FireWire (obsolete) | Linux user with FireWire hardware |
| Echo Audiofire4 | FireWire | 4 Out + SPDIF | Professional quality, DBUS controls | Requires FireWire (obsolete) | Advanced user wanting maximum flexibility |
...
The Edirol FA-101 interface presents its controls on the interface itself, unlike the Echo Audiofire4, which has many controls over DBUS. A brief list of some of them can be found in [Short guide of DBUS commands for FFADO devices](dbus_for_ffado.txt)

### USB interfaces: more modern and simpler

Locating the USB audio interface under GNU/Linux is straightforward:
...
Screenshots of how a Focusrite Scarlett interface is configured with alsa-scarlett-gui are included, to serve as a guide for other users:

Global interface configuration. In my case the Scarlett is synchronized to the external SPDIF clock.
...
To finish documenting this section, the case of using the Behringer UMC204HD USB interface is shown.
...
Being a stereo system, the input signal will be two channels, left and right. Anyone with more than one source will need a preamplifier to switch between them, or to connect and disconnect cables, or to have an interface with several line inputs. In my home installation, the only sound source is the television, connected by bluetooth to a simple receiver but with SPDIF output (coaxial and optical); that is why the input is channels capture_5 and capture_6, which correspond to the coaxial SPDIF input of the Scarlett 6i6.
...
From my experience I recommend that, starting from an operational setup with a sound that is satisfactory, you simply dedicate time to enjoying the music. When NatAmbio disappears from our conscious attention and all attention returns to the music, the goal has been achieved. At that moment we are no longer listening to an audio system: we are simply enjoying our favorite recordings.
