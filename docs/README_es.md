...
- [Parametrización del operador G en NatAmbio XTC](xtc/parametrizacion_en_XTC.md): cómo se derivan los cuatro parámetros del XTC a partir de bases de datos HRTF públicas, y cómo ajustarlos por escucha
...
- [python_nae_natambio](../tools/python_nae_natambio/README_es.md) Script python para la aplicación de NAE offline sobre ficheros wav. Posibilita pruebas específicas sobre los algoritmos NAE. Genera gráficas analíticas como las que presenta [NatAmbio Ambient Extractor (NAE)](nae/nae_es.md) 
...
- [xtc_filters](../tools/xtc_filters/README_es.md) Dos herramientas escritas en c que implementan el [Diseño de un cancelador de diafonía estéreo (XTC) por convolución para NatAmbio](xtc/xtc_filters_es.md) para su uso independiente de natambio, generando los filtros FIR XTC para su uso en otro convolver: `natambio-xtc-filters` para disposiciones simétricas, y `natambio-xtc-filters-asym` para el [modelo asimétrico](xtc/xtc_no_simetrico_es.md), que escribe un filtro directo y un cruzado por canal. Los parámetros se dan en ficheros TOML, con cuatro ejemplos comentados incluidos.
- [python_xtc_filters](../tools/python_xtc_filters/README_es.md) Contrapartidas en Python puro de los dos generadores en c, simétrico y asimétrico, que leen los mismos ficheros TOML. Reimplementan la tubería en NumPy/SciPy para poder ejecutarse sin compilar la cadena de herramientas c — útil para experimentar, enseñar y contrastar la salida de la versión c.
