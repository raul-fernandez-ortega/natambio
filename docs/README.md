...
- [Parametrization of the operator G in NatAmbio XTC](xtc/XTC_parametrization_en.md): how the four XTC parameters are derived from public HRTF databases, and how to tune them by listening.
...
- [xtc_filters](../tools/xtc_filters/README.md) Two C tools that implement the [design of a stereo crosstalk canceller (XTC) by convolution for NatAmbio](xtc/xtc_filters_en.md) for standalone use outside natambio, generating the XTC FIR filters for use in another convolver: `natambio-xtc-filters` for symmetric layouts, and `natambio-xtc-filters-asym` for the [asymmetric model](xtc/xtc_no_simetrico_en.md), which writes one direct filter and one cross filter per channel. Parameters are given in TOML files, with four annotated examples included.
- [python_xtc_filters](../tools/python_xtc_filters/README.md) Pure-Python counterparts of the two C generators, symmetric and asymmetric, reading the very same TOML files. They reimplement the pipeline in NumPy/SciPy so it runs without building the C toolchain — useful for experimentation, teaching, and cross-checking the C output.
