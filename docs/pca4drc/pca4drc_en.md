...
## Aligning the impulse responses: from the peak to maximum correlation

The conditioning described above centres each impulse response on its peak, and that decision pursues a specific aim: to remove the geometric effect of the distance to the loudspeaker so that PCA gathers the direct impulse into its principal component. But peak centring has a limitation worth making explicit, because it is not negligible: **a single sample decides it**, and the result is quantized to the sampling grid. Half a sample at 48 kHz is 10 µs, which at 16 kHz amounts to 60 degrees of phase. Whatever remains unaligned after peak centring enters the covariance in full, and PCA has no way of telling it apart from a genuine acoustic difference.

There is an alternative that starts from that same centring and refines it to a fraction of a sample: **maximizing the correlation between impulse responses**. Instead of entrusting everything to the position of one sample, the delay that maximizes the covariance between each pair in the set is sought, so that the whole waveform takes part rather than a single point. This has a further practical consequence: an intense early reflection close to the direct sound can move the sample peak to the wrong arrival, whereas correlation, weighing the entire window, is not so easily misled.

The procedure is as follows. For every pair of impulse responses the cross-correlation is computed and its maximum located, refined below the sample by parabolic interpolation of the peak. This yields an antisymmetric matrix of pairwise delays that must be reduced to one delay per impulse response, which is solved by least squares with a zero-sum constraint; its closed form turns out to be the row mean. The reason for using **all pairs** rather than taking one impulse response as a reference is to prevent a single faulty measurement from dragging the whole set with it. Finally the shift is applied with linear phase, which is exact for a band-limited signal and the only way to correct the fraction of a sample that peak centring cannot reach.

Two bounds keep the method within reason. The correlation window only has to cover the direct sound and the first reflections -- on real measurements, taking 20 ms or 200 ms gives the same delays to the second decimal place -- and a limit on the admissible delay prevents a pathological correlation from carrying an impulse response to a different arrival. Since the starting point is already the peak centring, the residual delays being corrected are, by construction, fractions of a sample.

Measured on a real four-way system with 16 positions per way, the residual delays range from 0.26 to 0.61 samples rms. The effect on the decomposition is appreciable: the variance explained by the principal component rises from 41-61 % to 50-67 %, while the second component falls from around 10 % to around 4 %. Put another way, **a good part of what appeared to be a second acoustic mode was nothing but sub-sample misalignment**. This result reinforces the interpretation proposed in this article: the principal component gathers what is common to the set, and everything withheld from it by conditioning defects ends up appearing as spurious structure in the components that follow.

It is worth being equally explicit about what this alignment does **not** achieve. It does not bring the principal component closer to the power average of the set. From one position to another the room does not change by delays but by different modal nulls and reflection patterns, and no delay applied impulse response by impulse response turns a coherent sum into a power average. The difference between the two quantities is a property of the acoustic field, not an alignment defect.

Finally, the process makes it possible to accompany the result with a figure for the mean and minimum similarity between pairs. It is worth stressing that this is **descriptive of the set, not a pass criterion**: on real measurements it runs from around 0.7 between close positions to around 0.4 between distant ones, a range within which a faulty set can also fall. As already argued when discussing homogeneity and coherence, a low correlation does not on its own disqualify a set of measurements; it describes its spread, which is a different matter.

## Loss and restoration of the relative level between ways

There is a consequence of the method that goes unnoticed while working with a single loudspeaker and that becomes decisive as soon as the system has several ways: **the PCA + DRC chain destroys the relative level information between them**.

The origin lies in the nature of the decomposition itself. An eigenvector defines a direction, not a magnitude: the principal component is determined only up to an arbitrary scale factor. And that factor is **not the same for each way**, because it depends on the covariance structure of that particular way's set of impulse responses -- the weights PCA assigns to each measurement -- on the eventual peak normalization when saving the components, and on the mean subtraction and windowing applied during conditioning. DRC then normalizes its own internal stages in turn, so that the filter it finally delivers retains no memory of the level at which the way that produced it plays.

The result is that, if four ways are measured and all four are put through the complete chain, their relative levels stop meaning anything. On the real four-way system used as a reference, the arbitrary factor varied by **5 dB** between one way and another. A mismatch of that size between the ways of a single loudspeaker is no nuance: it destroys the tonal balance that the equalization itself set out to achieve.

It should be noted that none of the normalizations DRC offers solves this, and not through any shortcoming of the tool. Its normalization criteria are anti-clipping mechanisms computed from the filter itself: they guarantee that the filter will not clip, not that its gain means the same as that of another filter obtained from another set of measurements. These are different problems and the solution to one cannot be the solution to the other.

The reference must therefore be restored **on the finished filters**, and the information needed to do so is in the measurements themselves. If the impulse responses of every way were captured with the same chain and the same gains -- a condition the experimental design must guarantee, and an easy one to meet -- then they **carry a level marker that is comparable between ways**. It only has to be exploited.

The criterion is to scale each filter so that **the in-band power its way delivers after correction equals the one it delivered when measured**. Both quantities come from the same source: the response measured at each position, and that measured response convolved with the filter. With that, the relative levels between ways after correction are exactly the ones the system had before it.

There is an averaging decision worth making explicit, because it is not immaterial. In reducing each way's measurements to a single number one can average the per-position differences expressed in dB, or average the powers and subtract afterwards. The first weights every point alike; the second weights by the energy delivered. On real measurements the two disagree by more than a decibel, and **the power average** is adopted here, which is consistent with the very definition of the reference band and answers the pertinent question: how much energy this way contributes to the listening region.

The reference band must lie inside the passband of every way compared; 200 to 2000 Hz is a reasonable choice for full-range ways. The choice of the energy average over that band has a precise motive besides: it is exactly the gain the filter applies to noise limited to the band, and that is what makes it comparable between ways whose responses differ greatly in shape.

It is worth setting out why this criterion is exact rather than approximate. An apparently simpler alternative would be to normalize each filter so that its energy-average gain over the band is 0 dB, without recourse to the measurements. But that **assumes the filter is flat inside the band**, and it is not: a correction filter is roughly the inverse of the response it corrects, so the two are anti-correlated and the residual depends on how much each way's response varies. On the real four-way system, that criterion left up to **1.9 dB** of mismatch between ways, whereas taking the reference from the measurements leaves it at zero by construction.

The scope of what is restored should be stated plainly: **the reference is the balance the system had at the time of measurement**. If the ways were not balanced then, the procedure perpetuates that imbalance. This is no worse than any alternative -- none can invent a reference that was never measured -- but it has one virtue: it turns the reference into a concrete, auditable measurement campaign instead of an assumed property of the filter.

Taken as a whole, this section describes a real limitation of the proposed procedure, not an implementation detail. PCA answers the question of **what shape** the response representative of a listening region has, and it does so well; but by construction it does not answer **at what level** it plays. Both questions have to be settled separately, and the design of the procedure must make this explicit so that whoever applies it does not take as preserved a piece of information that has in fact been lost along the way.

## Implications for crosstalk cancellation (XTC)

When the equalized system incorporates crosstalk cancellation, relative level stops being a matter of tonal balance and becomes an operating requirement. Two consequences of the method described are therefore worth pointing out, one favourable and one that imposes a limit.

The first is that **the level balance between the two loudspeakers of each dipole directly conditions the cancellation ceiling that can be reached**. The asymmetric XTC model developed for NatAmbio incorporates a balance factor between channels, from which it follows that the residual crosstalk is, to first order, proportional to the departure of that balance from unity. In figures: a balance error of 1 dB puts the ceiling at about 19 dB of cancellation, 2 dB brings it down to about 14 dB and 3 dB to about 11 dB, well below what the parametric model can deliver. The practical tolerance is therefore under 1 dB.

This turns the level restoration described in the previous section into a requirement rather than a refinement. And the error to look at is the one **within each dipole**, which is what XTC sees, not the overall error across all ways. On the four-way system used as a reference, the approximate criterion left 0.05 dB of balance error in the front dipole and 0.64 dB in the rear one: both within tolerance, but by chance more than by design, since nothing bounds that residual. The larger mismatch that criterion produced, 1.5 dB, was between the front pair and the rear one, which affects the relative level of the ambient component but not the operation of each dipole's XTC. Taking the reference from the measurements themselves makes the balance exact in both cases.

The second consequence is a limitation of substance, and it is important not to pass over it. **A zone equalization does not necessarily provide the phase alignment XTC needs.** Crosstalk cancellation requires the antisignal to reach the opposite ear with the correct temporal relation to the signal it sets out to cancel, and that relation is a property of one specific listening point, not of a region. The procedure described here does precisely the opposite: it centres each impulse response on its peak, deliberately removing the time-of-flight differences between positions, and then averages over a region where phase varies strongly from one point to another. The resulting filter is, besides, minimum phase, so it corrects magnitude and makes no attempt at correcting excess phase.

Both procedures should therefore be understood as complementary rather than as substitutes. PCA zone equalization gives XTC two things it needs: a magnitude response corrected over the listening region and, with the restoration described, an exact level balance between the loudspeakers of the dipole. What it does not give is the fine temporal alignment between the two acoustic paths, which remains a property of the listening point and has to be handled by other means: in the case of NatAmbio, through the parameters of the XTC model itself, adjusted by listening. Anyone combining the two techniques should know that they are solving two different problems with two different tools, and that the goodness of the zone equalization does not by itself guarantee the quality of the cancellation.
