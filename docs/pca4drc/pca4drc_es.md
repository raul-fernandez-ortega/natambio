...
## Alineado de las impulsivas: del pico al máximo de correlación

El acondicionamiento descrito más arriba centra cada impulsiva en su pico, y esa decisión persigue un objetivo concreto: eliminar el efecto geométrico de la distancia al altavoz para que PCA recoja el impulso directo en su componente principal. Pero el centrado por pico tiene una limitación que conviene explicitar, porque no es despreciable: **decide con una sola muestra**, y el resultado queda cuantizado a la rejilla de muestreo. Media muestra a 48 kHz son 10 µs, que a 16 kHz equivalen a 60 grados de fase. Lo que queda sin alinear después de centrar por pico entra íntegramente en la covarianza, y PCA no tiene forma de distinguirlo de una diferencia acústica real.

Existe una alternativa que parte de ese mismo centrado y lo afina a fracción de muestra: **maximizar la correlación entre impulsivas**. En lugar de fiarlo todo a la posición de una muestra, se busca el retardo que maximiza la covarianza entre cada pareja del banco, con lo que interviene la forma de onda completa y no un único punto. Eso tiene además una consecuencia práctica: una reflexión temprana intensa y próxima al sonido directo puede desplazar el pico de muestra a un arribo equivocado, mientras que la correlación, al pesar toda la ventana, no se deja engañar con la misma facilidad.

El procedimiento concreto es el siguiente. Para cada pareja de impulsivas se calcula la correlación cruzada y se localiza su máximo, refinándolo por debajo de la muestra mediante interpolación parabólica del pico. Esto produce una matriz antisimétrica de retardos por parejas que hay que reducir a un retardo por impulsiva, lo cual se resuelve por mínimos cuadrados con la restricción de suma nula; su forma cerrada resulta ser la media por filas. La razón de usar **todas las parejas** en lugar de tomar una impulsiva como referencia es evitar que una sola medida defectuosa arrastre al conjunto entero. Finalmente el desplazamiento se aplica con fase lineal, que es exacto para una señal limitada en banda y la única forma de corregir la fracción de muestra que el centrado por pico no alcanza.

Dos acotaciones mantienen el método dentro de lo razonable. La ventana de correlación solo tiene que cubrir el sonido directo y las primeras reflexiones —en medidas reales, tomar 20 ms o 200 ms da los mismos retardos hasta la segunda cifra decimal— y un tope al retardo admisible impide que una correlación patológica lleve una impulsiva a un arribo distinto. Como el punto de partida ya es el centrado por pico, los retardos residuales que se corrigen son, por construcción, fracciones de muestra.

Medido sobre un sistema real de cuatro vías con 16 posiciones por vía, los retardos residuales van de 0,26 a 0,61 muestras rms. El efecto sobre la descomposición es apreciable: la varianza explicada por la componente principal sube del 41–61 % al 50–67 %, mientras la segunda componente cae de alrededor del 10 % a alrededor del 4 %. Dicho de otro modo, **buena parte de lo que aparentaba ser un segundo modo acústico no era más que desalineamiento sub-muestra**. Es un resultado que refuerza la interpretación propuesta en este artículo: la componente principal recoge lo común al banco, y todo lo que se le sustrae por defectos de acondicionamiento acaba apareciendo como estructura espuria en las componentes siguientes.

Conviene ser igual de explícito con lo que este alineado **no** consigue. No acerca la componente principal al promedio en potencia del conjunto. De una posición a otra la sala no cambia por retardos sino por nulos modales y patrones de reflexión distintos, y ningún retardo aplicado impulsiva a impulsiva convierte una suma coherente en una media de potencia. La diferencia entre ambas magnitudes es una propiedad del campo acústico, no un defecto de alineado.

Por último, el proceso permite acompañar el resultado de una cifra de similitud media y mínima entre parejas. Merece la pena subrayar que es **descriptiva del banco, no un criterio de aprobado**: en medidas reales va de alrededor de 0,7 entre posiciones próximas a alrededor de 0,4 entre lejanas, un intervalo en el que también puede caer un conjunto defectuoso. Como ya se ha argumentado al hablar de homogeneidad y coherencia, una correlación baja no descalifica por sí sola un banco de medidas; describe su dispersión, que es cosa distinta.

## Pérdida y restitución del nivel relativo entre vías

Hay una consecuencia del método que no se aprecia mientras se trabaja con un solo altavoz y que se vuelve determinante en cuanto el sistema tiene varias vías: **la cadena PCA + DRC destruye la información de nivel relativo entre ellas**.

El origen está en la propia naturaleza de la descomposición. Un autovector define una dirección, no una magnitud: la componente principal queda determinada salvo un factor de escala arbitrario. Y ese factor **no es el mismo para cada vía**, porque depende de la estructura de covarianza del banco de impulsivas de esa vía concreta —los pesos que PCA asigna a cada medida—, de la eventual normalización por pico al guardar las componentes, y de la resta de media y el ventaneado aplicados en el acondicionamiento. Después, DRC normaliza a su vez sus propias etapas internas, de modo que el filtro que finalmente entrega no conserva ninguna memoria del nivel al que suena la vía que lo originó.

El resultado es que, si se miden cuatro vías y se pasan las cuatro por la cadena completa, sus niveles relativos dejan de significar nada. En el sistema real de cuatro vías utilizado como referencia, el factor arbitrario variaba **5 dB** entre unas y otras. Un desajuste de esa magnitud entre las vías de un mismo altavoz no es un matiz: destruye el equilibrio tonal que la propia ecualización pretendía conseguir.

Conviene señalar que ninguna de las normalizaciones que ofrece DRC resuelve esto, y no por un defecto de la herramienta. Sus criterios de normalización son mecanismos anticlipping calculados a partir del propio filtro: garantizan que el filtro no sature, no que su ganancia signifique lo mismo que la de otro filtro obtenido de otro banco de medidas. Son problemas distintos y la solución de uno no puede serlo del otro.

La referencia hay que restituirla, por tanto, **sobre los filtros ya terminados**, y la información necesaria para hacerlo está en las propias medidas. Si las impulsivas de todas las vías se han captado con la misma cadena y las mismas ganancias —condición que el diseño experimental debe garantizar, y que es fácil de cumplir—, entonces **llevan incorporada una marca de nivel comparable entre vías**. Basta explotarla.

El criterio es escalar cada filtro para que **la potencia en banda que su vía entrega después de corregir iguale la que entregaba al medir**. Ambas magnitudes se obtienen de lo mismo: la respuesta medida en cada posición, y la respuesta medida convolucionada por el filtro. Con eso, los niveles relativos entre vías después de la corrección son exactamente los que el sistema tenía antes de ella.

Hay una decisión de promediado que conviene explicitar, porque no es indiferente. Al reducir las medidas de cada vía a un solo número se puede promediar las diferencias por posición expresadas en dB, o promediar las potencias y restar después. La primera pesa igual cada punto; la segunda pesa por energía entregada. Sobre medidas reales las dos discrepan en más de un decibelio, y aquí se adopta **el promedio de potencias**, que es coherente con la propia definición de la banda de referencia y responde a la pregunta pertinente: cuánta energía aporta esta vía a la zona de escucha.

La banda de referencia debe caer dentro del paso de todas las vías comparadas; de 200 a 2000 Hz es una elección razonable para vías de banda ancha. La elección de la media en energía sobre esa banda tiene además un motivo preciso: es exactamente la ganancia que el filtro aplica a ruido limitado a la banda, y eso es lo que la hace comparable entre vías cuyas respuestas tienen formas muy distintas.

Merece la pena señalar por qué este criterio es exacto y no aproximado. Una alternativa aparentemente más simple sería normalizar cada filtro para que su ganancia media en energía sobre la banda fuese 0 dB, sin recurrir a las medidas. Pero eso **supone que el filtro es plano dentro de la banda**, y no lo es: un filtro de corrección es aproximadamente el inverso de la respuesta que corrige, de modo que ambos están anticorrelados y el residuo depende de cuánto varíe la respuesta de cada vía. Sobre el sistema real de cuatro vías, ese criterio dejaba hasta **1.9 dB** de desajuste entre vías, mientras que tomar la referencia de las medidas lo deja en cero por construcción.

Conviene ser explícito sobre el alcance de lo que se restituye: **la referencia es el equilibrio que el sistema tenía en el momento de medir**. Si las vías no estaban equilibradas entonces, el procedimiento perpetúa ese desequilibrio. Esto no es peor que cualquier alternativa —ninguna puede inventar una referencia que no se haya medido— pero sí tiene una virtud: convierte la referencia en una campaña de medidas concreta y auditable, en lugar de una propiedad supuesta del filtro.

Visto en conjunto, este apartado describe una limitación real del procedimiento propuesto, no un detalle de implementación. PCA responde a la pregunta de **qué forma** tiene la respuesta representativa de una región de escucha, y lo hace bien; pero por construcción no responde a **a qué nivel** suena. Ambas cuestiones hay que resolverlas por separado, y el diseño del procedimiento debe hacerlo explícito para que quien lo aplique no dé por conservada una información que en realidad se ha perdido por el camino.

## Implicaciones para la cancelación de diafonía (XTC)

Cuando el sistema ecualizado incorpora cancelación de diafonía, el nivel relativo deja de ser una cuestión de equilibrio tonal para convertirse en un requisito de funcionamiento. Conviene por tanto señalar dos consecuencias del método descrito, una favorable y otra que impone un límite.

La primera es que **el equilibrio de nivel entre los dos altavoces de cada dipolo condiciona directamente el techo de cancelación alcanzable**. El modelo XTC asimétrico desarrollado para NatAmbio incorpora un factor de balance entre canales, y de él se deduce que la diafonía residual es, en primer orden, proporcional a la desviación de ese balance respecto de la unidad. En cifras: un error de balance de 1 dB sitúa el techo en unos 19 dB de cancelación, 2 dB lo bajan a unos 14 dB y 3 dB a unos 11 dB, muy por debajo de lo que el modelo paramétrico puede entregar. La tolerancia práctica está, por tanto, por debajo de 1 dB.

Esto convierte la restitución de nivel descrita en el apartado anterior en un requisito y no en un refinamiento. Y conviene mirar el error **dentro de cada dipolo**, que es el que XTC ve, y no el error global entre todas las vías. En el sistema de cuatro vías utilizado como referencia, el criterio aproximado dejaba 0.05 dB de error de balance en el dipolo frontal y 0.64 dB en el trasero: ambos dentro de tolerancia, pero por casualidad más que por diseño, porque nada acota ese residuo. El desajuste mayor que ese criterio producía, 1.5 dB, era entre el par frontal y el trasero, lo cual afecta al nivel relativo de la componente ambiental pero no al funcionamiento del XTC de cada dipolo. Tomar la referencia de las propias medidas hace el balance exacto en los dos casos.

La segunda consecuencia es una limitación de fondo, y es importante no pasarla por alto. **Una ecualización de zona no proporciona necesariamente el ajuste de fase que XTC necesita.** La cancelación de diafonía exige que la antiseñal llegue al oído contrario con la relación temporal correcta respecto de la señal que pretende cancelar, y esa relación es una propiedad de un punto de escucha concreto, no de una región. El procedimiento aquí descrito hace justamente lo contrario: centra cada impulsiva en su pico, con lo que elimina deliberadamente las diferencias de tiempo de vuelo entre posiciones, y después promedia sobre una región donde la fase varía fuertemente de un punto a otro. El filtro resultante, además, es de fase mínima, de modo que corrige magnitud y no intenta corregir la fase en exceso.

De ahí que ambos procedimientos deban entenderse como complementarios y no como sustitutivos. La ecualización de zona por PCA aporta a XTC dos cosas que necesita: una respuesta de magnitud corregida sobre la región de escucha y, con la restitución descrita, un balance de nivel exacto entre los altavoces del dipolo. Lo que no aporta es la alineación temporal fina entre los dos caminos acústicos, que sigue siendo una propiedad del punto de escucha y ha de tratarse por otros medios: en el caso de NatAmbio, mediante los parámetros del propio modelo XTC ajustados por escucha. Quien combine ambas técnicas debe saber que está resolviendo dos problemas distintos con dos herramientas distintas, y que la bondad de la ecualización de zona no garantiza por sí sola la calidad de la cancelación.
